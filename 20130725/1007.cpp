...
struct point3{
    ...
    double x,y,z; void read() { scanf("%lf%lf%lf", &x, &y, &z); } };
struct line3{ 
    line3(){} 
...
bool check(int a, int b, double &ans) {
    double r1 = distance(circle[a].a, circle[a].b);
    double r2 = distance(circle[b].a, circle[b].b);
    point3 p1 = circle[a].a + pvec(circle[a]);
    point3 p2 = circle[b].a + pvec(circle[b]);
    line3 l1 = line3(circle[a].a, p1);
    line3 l2 = line3(circle[b].a, p2);
    double dis = linetoline(l1, l2);
    if (dis - r1 - r2 < eps) return true;
    ans = min(ans, dis - r1 - r2);
    return false;
}
